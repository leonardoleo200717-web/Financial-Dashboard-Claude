...
(function () {
  ...
  function render() {
    ...
    if (E.monthSeries(data).length === 0 && activeTab !== 'impostazioni' && activeTab !== 'simulatore' && activeTab !== 'tasse' && activeTab !== 'importa') {
    ...
    switch (activeTab) {
      ...
      case 'importa': renderImporta(main); break;
  ...
  const chartTitles = {
    ...
    'c-imp-expenses': 'Spese vs Reddito', 'c-imp-invest': 'Flussi di investimento',
    'c-imp-salary': 'Andamento stipendio', 'c-imp-categories': 'Categorie di spesa',
  };
  const fmt = E.fmtEUR;
  const fmtP = E.fmtPct;
  ...
  function renderAndamento(main) {
    ...
    main.appendChild(netWorthBreakdownCard());
  ...
  /* Conto per conto, quanto fa il patrimonio di questo mese. Esiste per una
     ragione precisa: quando il totale non torna con il proprio foglio, la
     domanda è sempre "quale conto sta sballando?" — e senza questa scheda
     l'unico modo per rispondere era aprire Impostazioni e sommare a mano.
     Le pensioni sono elencate SOTTO il totale liquido, mai dentro. */
  function netWorthBreakdownCard() {
    const ym = lastMonthWith(currentMonth());
    const rows = [];
    let liquidSum = 0, lockedSum = 0;
    (data.accounts || []).forEach(a => {
      const s = E.getSnapshot(data, a.id, ym);
      if (!s || s.balancePayday == null) return;
      const liquid = E.isLiquid(a);
      if (liquid) liquidSum += s.balancePayday; else lockedSum += s.balancePayday;
      rows.push({ a, bal: s.balancePayday, liquid });
    });

    // Stesso conto bancario inserito due volte = patrimonio gonfiato. È il
    // modo più facile di sbagliarsi (import ripetuto, conto ricreato dal
    // wizard), quindi va detto qui, dove si guarda il totale.
    const byName = {};
    rows.forEach(r => { const k = r.a.name.trim().toLowerCase(); (byName[k] = byName[k] || []).push(r); });
    const dupes = Object.values(byName).filter(g => g.length > 1);

    const body = el('div');
    let html = `<div class="kpi"><span>Patrimonio liquido — ${E.monthLabelIT(ym)}</span><b style="font-size:22px">${fmt(E.r2(liquidSum))}</b></div>
      <table class="data-table" style="width:100%;margin-top:8px"><tbody>`;
    rows.filter(r => r.liquid).forEach(r => {
      html += `<tr><td>${escapeHtml(r.a.name)} <span class="badge">${r.a.type}</span></td><td style="text-align:right">${fmt(r.bal)}</td></tr>`;
    });
    html += `<tr style="font-weight:700;border-top:2px solid var(--line)"><td>Totale liquido</td><td style="text-align:right">${fmt(E.r2(liquidSum))}</td></tr>`;
    html += '</tbody></table>';
    if (lockedSum > 0) {
      html += `<p class="muted small" style="margin:10px 0 0">Pensioni (<b>${fmt(E.r2(lockedSum))}</b>) — <b>escluse</b> dal totale qui sopra e da tutti i calcoli FIRE prima dei ${data.settings.fire.pensionStartAge} anni. Compaiono solo nella linea tratteggiata "Totale (incl. pensioni)" del grafico e nella tab Pensioni.</p>`;
    }
    if (dupes.length) {
      html += `<p class="small" style="color:var(--neg);margin:10px 0 0">⚠ <b>Conti duplicati</b>: ${dupes.map(g => escapeHtml(g[0].a.name) + ' (×' + g.length + ')').join(', ')}. Lo stesso conto è contato più volte, quindi il patrimonio qui sopra è gonfiato. Vai in <b>Impostazioni → Conti</b> ed elimina la copia di troppo.</p>`;
    }
    body.innerHTML = html;
    return card('Composizione del patrimonio', body,
      'Il dettaglio conto per conto del mese più recente. Il <b>totale liquido</b> è la cifra da confrontare con il tuo foglio: somma corrente, depositi e broker, e <b>non</b> include le pensioni. Se il numero non torna, quasi sempre è un conto duplicato o un conto che non dovrebbe esserci (es. un conto comune/bridge): li vedi elencati qui.');
  }
  ...
  function drawNetWorth(months) {
    ...
    const fire = E.fireNumberTwoPhase(data.settings.fire).twoPhase; // same pension-aware target as the FIRE tab
    ...
    makeChart('c-networth', {
      ...
      data: {
        ...
        datasets: [
          ...
          { label: 'FIRE number (due fasi)', data: months.map(() => fire), borderColor: NEG, borderDash: [2, 2], pointRadius: 0, fill: false },
        ],
      },
      options: withMilestones(baseLineOpts(), months),
    });
  }
  ...
  function drawMonthlyGrowth(months) {
    ...
    makeChart('c-monthly', {
      ...
      options: Object.assign(withMilestones(baseLineOpts(), months), {
        plugins: {
  ...
  function drawSavings(months) {
    ...
    makeChart('c-savings', {
      ...
      options: withMilestones(pctLineOpts(), months),
    });
  }
  ...
  function drawAllocation(months, showArchived) {
    ...
    makeChart('c-alloc', {
      ...
      options: Object.assign(withMilestones(baseLineOpts(), months), { scales: { y: { stacked: true, ticks: { callback: v => fmt(v) } } } }),
    });
  }
  ...
  function renderStorico(main) {
    ...
    const tb = el('tbody');
    // Annual rollup rows (spec Tab 2): rows are newest-first, so emit the year
    // summary when we first enter each year.
    const chrono = E.buildMonthlyTable(data); // ascending, for year aggregates
    let lastYear = null;
    rows.forEach(r => {
      const yr = r.ym.slice(0, 4);
      if (yr !== lastYear) {
        tb.appendChild(yearRollupRow(yr, chrono));
        lastYear = yr;
      }
      const tr = el('tr');
  ...
  // Bold year-summary row: Δ anno vs end of previous year (+ YoY %), summed
  // contributions/market, expenses over unflagged months, mean savings rate.
  function yearRollupRow(yr, chrono) {
    const inYear = chrono.filter(r => r.ym.slice(0, 4) === yr);
    const prevYear = chrono.filter(r => r.ym.slice(0, 4) < yr);
    const endLiquid = inYear.length ? inYear[inYear.length - 1].liquid : null;
    const startLiquid = prevYear.length ? prevYear[prevYear.length - 1].liquid : null;
    const delta = (endLiquid != null && startLiquid != null) ? E.r2(endLiquid - startLiquid) : null;
    const yoy = (delta != null && startLiquid) ? delta / startLiquid : null;
    const sum = (get) => E.r2(inYear.reduce((s, r) => s + (get(r) || 0), 0));
    const contributions = sum(r => r.contributions);
    const market = sum(r => r.market);
    const expenses = E.r2(inYear.filter(r => !(r.flags || []).length && r.expenses != null)
      .reduce((s, r) => s + r.expenses, 0));
    const srVals = inYear.map(r => r.savingsRate).filter(v => v != null);
    const savingsRate = srVals.length ? srVals.reduce((a, b) => a + b, 0) / srVals.length : null;
    const tr = el('tr', 'year-row');
    tr.innerHTML = `
      <td><b>${yr}</b></td>
      <td><b>${fmt(endLiquid)}</b></td>
      <td class="${delta >= 0 ? 'pos' : 'neg'}"><b>${delta == null ? '—' : fmt(delta)}</b></td>
      <td><b>${yoy == null ? '—' : fmtP(yoy)}</b></td>
      <td><b>${fmt(contributions)}</b></td>
      <td><b>${fmt(market)}</b></td>
      <td><b>${fmt(expenses)}</b></td>
      <td><b>${savingsRate == null ? '—' : fmtP(savingsRate)}</b></td>
      <td></td>`;
    return tr;
  }
  function expCell(r) {
  ...
  function renderFire(main) {
    ...
    // 3.2 Coast FIRE — pension-aware: targets the two-phase number (PME/AOW/
    // Generali reduce the capital you actually need), so progress isn't
    // under-reported against the pension-blind simple number.
    const today = currentMonth();
    const liquid = E.fireCapital(data, lastMonthWith(today)) || 0;
    const age = E.ageAt(data.settings.birthDate, today);
    const coastTarget = tp.twoPhase;
    const coastBase = E.coastFire(f, liquid, age, f.realReturnBase, coastTarget);
    const coastOpt = E.coastFire(f, liquid, age, f.realReturnOptimistic, coastTarget);
    ...
    coastBody.innerHTML = `
      ...
      <div class="kpi"><span>Obiettivo (due fasi, con pensioni)</span><b>${fmt(coastTarget)}</b></div>
      <div class="kpi"><span>Capitale che basterebbe oggi (${(f.realReturnBase * 100).toFixed(0)}% reale)</span><b>${fmt(coastBase.requiredToday)}</b></div>
    ...
    coastBody.appendChild(canvas('c-coast'));
    grid.appendChild(card('3.2 Coast FIRE', coastBody, INFO_FIRE.coast));
    setTimeout(() => drawCoastCrossover(liquid, age, f, coastTarget), 0);
  ...
  // progress bar, plus Coast status. Targets the TWO-PHASE number — pensions
  // reduce the capital actually needed, and the primary metric should say so.
  function fireHeadline() {
    const f = data.settings.fire;
    const today = currentMonth();
    const cap = E.fireCapital(data, lastMonthWith(today)) || 0;
    const tp = E.fireNumberTwoPhase(f);
    const fireN = tp.twoPhase;
    ...
    const coast = E.coastFire(f, cap, age, f.realReturnBase, fireN);
    const coasting = cap >= coast.requiredToday;
    const body = el('div');
    body.innerHTML = `
      <div class="headline-row">
        <div><div class="muted small">Capitale FIRE</div><div class="big">${fmt(cap)}</div></div>
        <div><div class="muted small">FIRE number (con pensioni)</div><div class="big">${fmt(fireN)}</div></div>
      ...
      <div class="muted small">Obiettivo a due fasi (le pensioni riducono il fabbisogno di ${fmt(tp.pensionSaving)}; versione semplice: ${fmt(tp.simple)}). Ipotesi: versi <b>${fmt(monthly)}/mese</b> (mediana 12m) al ${(f.realReturnBase * 100).toFixed(1)}%.</div>`;
    return card('🔥 A che punto sei', body, INFO_FIRE.headline);
  }

  // Coast crossover: capital compounding with ZERO further contributions vs
  // the target line — the crossing age is the "you could stop today" answer.
  function drawCoastCrossover(cap, age, f, target) {
    if (!$('#c-coast')) return;
    const years = [];
    const balBase = [], balOpt = [];
    let b1 = cap, b2 = cap;
    const horizon = Math.max(5, Math.ceil((f.fireAge - (age || 40)) + 15));
    for (let y = 0; y <= horizon; y++) {
      years.push(age != null ? Math.round(age + y) : y);
      balBase.push(E.r2(b1)); balOpt.push(E.r2(b2));
      b1 *= (1 + f.realReturnBase); b2 *= (1 + f.realReturnOptimistic);
    }
    makeChart('c-coast', {
      type: 'line',
      data: {
        labels: years,
        datasets: [
          { label: 'Capitale senza nuovi versamenti (' + (f.realReturnBase * 100).toFixed(0) + '%)', data: balBase, borderColor: '#3498db', borderWidth: 2.5, pointRadius: 0, tension: .15 },
          { label: (f.realReturnOptimistic * 100).toFixed(0) + '%', data: balOpt, borderColor: '#27ae60', borderDash: [4, 3], pointRadius: 0, tension: .15 },
          { label: 'Obiettivo (due fasi)', data: years.map(() => target), borderColor: NEG, borderDash: [2, 2], pointRadius: 0 },
        ],
  ...
  function projectionCard() {
    ...
    const recompute = () => {
      ...
      const fireN = E.fireNumberTwoPhase(f).twoPhase; // pension-aware, same target as headline/coast
      const common = {
      ...
      makeChart('c-proj', {
        ...
        data: {
          ...
          datasets: [
            ...
            { label: 'FIRE number (due fasi)', data: labels.map(() => fireN), borderColor: NEG, borderDash: [2, 2], pointRadius: 0 },
          ],
        }, options: baseLineOpts(),
      });
    };
  ...
  function renderImpostazioni(main) {
    ...
    const params = [
      ...
      ['meanReturn', 'Monte Carlo — rendimento medio', f.monteCarlo.meanReturn, 'mc'],
      ['stdDev', 'Monte Carlo — volatilità (σ)', f.monteCarlo.stdDev, 'mc'],
    ];
    ...
    $$('[data-param]', p).forEach(inp => inp.onchange = () => {
      ...
      if (scope === 'fire') f[key] = v; else if (scope === 'mc') f.monteCarlo[key] = v; else data.settings[key] = v;
      save();
    });
  ...
  /* ------------------------- export / import -------------------------- */
  // Export payload: everything EXCEPT the AI API key, which must never leave
  // this browser (backups get shared/synced; the key does not belong in them).
  ...
  function doImport(ev) {
    ...
    reader.onload = () => {
      try {
        ...
        // Never accept a key from a file; keep the one already in this browser.
        if (migrated.settings && migrated.settings.ai) migrated.settings.ai.apiKey = (data.settings.ai && data.settings.ai.apiKey) || '';
        if (mode === 'merge') {
  ...
  function renderImporta(main) {
    ...
    cfgBody.innerHTML = `
      <p class="muted" style="margin:0 0 12px">Configura gli IBAN dei tuoi conti e delle persone con cui condividi spese. Questo permette di separare i trasferimenti interni dalle spese reali, e di escludere i contributi del partner.</p>
      <h4 style="margin:8px 0 4px">Tuoi IBAN (conti propri)</h4>
      <div id="imp-own-ibans"></div>
      <div class="form-row"><input id="imp-new-iban" placeholder="IBAN" style="flex:2">
        <select id="imp-new-iban-acc" style="flex:1">${data.accounts.map(a => `<option value="${a.id}">${escapeHtml(a.name)}</option>`).join('')}</select>
        <button class="btn small" id="imp-add-iban">+ Aggiungi</button></div>
      <h4 style="margin:12px 0 4px">IBAN spese condivise (partner)</h4>
      <p class="muted small" style="margin:0 0 8px">Crediti da questi IBAN non saranno contati come tuo reddito (es. contributo del partner al conto comune).</p>
      <div id="imp-shared-ibans"></div>
    ...
    function renderIbanLists() {
      const ownDiv = cfgBody.querySelector('#imp-own-ibans');
      ownDiv.innerHTML = '';
      Object.entries(imp.ownIbans).forEach(([iban, accId]) => {
        const acc = E.accountById(data, accId);
        const r = el('div', 'form-row');
        r.innerHTML = `<code>${escapeHtml(iban)}</code> <span class="badge">${acc ? escapeHtml(acc.name) : accId}</span> <button class="link neg imp-del-own" data-iban="${escapeHtml(iban)}">×</button>`;
        ownDiv.appendChild(r);
      });
      const sharedDiv = cfgBody.querySelector('#imp-shared-ibans');
      sharedDiv.innerHTML = '';
      Object.entries(imp.sharedExpenseIbans).forEach(([iban, label]) => {
        const r = el('div', 'form-row');
        r.innerHTML = `<code>${escapeHtml(iban)}</code> <span class="badge">${escapeHtml(label)}</span> <button class="link neg imp-del-shared" data-iban="${escapeHtml(iban)}">×</button>`;
        sharedDiv.appendChild(r);
      });
    }
    renderIbanLists();
    cfgBody.querySelector('#imp-add-iban').onclick = () => {
      const iban = cfgBody.querySelector('#imp-new-iban').value.replace(/\s/g, '').toUpperCase();
      const accId = cfgBody.querySelector('#imp-new-iban-acc').value;
      if (!iban) return;
      imp.ownIbans[iban] = accId; save(); renderIbanLists();
      cfgBody.querySelector('#imp-new-iban').value = '';
    };
    cfgBody.querySelector('#imp-add-shared').onclick = () => {
      const iban = cfgBody.querySelector('#imp-new-shared').value.replace(/\s/g, '').toUpperCase();
      const label = cfgBody.querySelector('#imp-new-shared-label').value || 'Partner';
      if (!iban) return;
      imp.sharedExpenseIbans[iban] = label; save(); renderIbanLists();
      cfgBody.querySelector('#imp-new-shared').value = '';
      cfgBody.querySelector('#imp-new-shared-label').value = '';
    };
    ...
    uploadBody.innerHTML = `
      <div class="upload-zone" style="display:flex;gap:16px;flex-wrap:wrap">
        <div class="upload-box" style="flex:1;min-width:200px;border:2px dashed var(--line);border-radius:var(--radius);padding:20px;text-align:center;cursor:pointer" id="imp-abn-zone">
          <div style="font-size:24px;margin-bottom:8px">🏦</div>
          <div><b>ABN AMRO</b></div>
          <div class="muted small">File XLS o TXT/TSV</div>
          <input type="file" accept=".xls,.xlsx,.txt,.tsv,.csv" id="imp-abn-file" style="display:none">
          <div id="imp-abn-status" class="muted small" style="margin-top:8px"></div>
        </div>
        <div class="upload-box" style="flex:1;min-width:200px;border:2px dashed var(--line);border-radius:var(--radius);padding:20px;text-align:center;cursor:pointer" id="imp-scal-zone">
          <div style="font-size:24px;margin-bottom:8px">📊</div>
          <div><b>Scalable Capital</b></div>
          <div class="muted small">File CSV transazioni</div>
          <input type="file" accept=".csv" id="imp-scal-file" style="display:none">
          <div id="imp-scal-status" class="muted small" style="margin-top:8px"></div>
        </div>
      </div>
      <div id="imp-preview" style="margin-top:16px"></div>`;
    main.appendChild(card('Carica estratti', uploadBody, 'Carica gli estratti conto scaricati dalla tua banca e da Scalable Capital. Il file ABN AMRO viene analizzato per estrarre stipendio, spese reali e trasferimenti interni. Il file Scalable mostra i versamenti e le operazioni sul broker.'));

    const abnFileInput = uploadBody.querySelector('#imp-abn-file');
    const scalFileInput = uploadBody.querySelector('#imp-scal-file');
    ...
    scalFileInput.onchange = function () {
      ...
      reader.onload = () => {
        try {
          importState.scalRows = E.parseScalableCSV(reader.result);
          if (!importState.scalRows.length) throw new Error('Nessuna riga valida');
          importState.scalDigest = E.scalableMonthlyDigest(importState.scalRows);
          uploadBody.querySelector('#imp-scal-status').innerHTML = `<span style="color:var(--pos)">✓ ${importState.scalRows.length} transazioni caricate</span>`;
          renderPreview(uploadBody.querySelector('#imp-preview'), main);
        } catch (e) { uploadBody.querySelector('#imp-scal-status').innerHTML = `<span style="color:var(--neg)">✗ ${escapeHtml(e.message)}</span>`; }
      };
      reader.readAsText(file);
    };
    ...
    // --- Insights card (always visible if we have data) ---
    if (E.monthSeries(data).length > 0) {
      renderInsights(main);
    }
  }
  ...
  function renderPreview(container, main) {
      ...
      mapSec.querySelectorAll('.imp-role').forEach(sel => {
        sel.onchange = () => {
          ...
          if (v === 'new') {
            ...
            // Un conto con lo stesso nome esiste già: crearne un secondo
            // significherebbe contare due volte lo stesso conto bancario nel
            // patrimonio. Riusiamo quello esistente invece di duplicarlo.
            const existing = data.accounts.find(x => x.name.trim().toLowerCase() === name.trim().toLowerCase());
            if (existing) {
              alert('Esiste già un conto chiamato "' + existing.name + '": collego l\'estratto a quello invece di crearne un secondo (altrimenti il patrimonio verrebbe contato due volte).');
              roles2[sel.dataset.acc] = { kind: 'own', accountId: existing.id };
              save(); recomputeAbnDigest(); render();
              return;
            }
          ...
          if (v.startsWith('own:')) {
            // Due conti dell'estratto sullo stesso conto della dashboard: i
            // saldi del secondo sovrascriverebbero quelli del primo.
            const taken = Object.entries(roles2).find(([accNum, r2v]) =>
              accNum !== sel.dataset.acc && r2v && r2v.kind === 'own' && r2v.accountId === v.slice(4));
            if (taken) {
              const other = E.accountById(data, v.slice(4));
              alert('Il conto "' + (other ? other.name : '?') + '" è già collegato al conto ' + taken[0] + ' di questo estratto. Ogni conto della dashboard può essere collegato a un solo conto dell\'estratto, altrimenti i saldi si sovrascrivono.');
    ...
    if (importState.abnDigest) {
      ...
      months.forEach(ym => {
        ...
        const existSnap = data.accounts.filter(a => a.type === 'current').map(a => E.getSnapshot(data, a.id, ym)).find(s => s);
        const hasExisting = !!existing;
        const cellCls = (field) => {
          if (!hasExisting) return '';
          return ' style="background:rgba(46,204,113,0.1)"';
        };
        html += `<tr>
          <td><b>${E.monthLabelIT(ym)}</b>${d.partial ? ' <span title="Ciclo parziale" style="color:var(--neg)">⚠</span>' : ''}</td>
          <td${cellCls()}><input type="number" class="imp-edit" data-ym="${ym}" data-field="balancePayday" value="${edits.balancePayday != null ? edits.balancePayday : (d.balancePayday != null ? d.balancePayday : '')}" style="width:90px"></td>
      ...
      digestSec.querySelectorAll('.imp-edit').forEach(inp => {
        inp.onchange = () => {
          ...
          importState.edits[ym][field] = inp.value === '' ? null : parseFloat(inp.value);
        };
      });
    ...
    if (importState.scalDigest) {
      ...
      months.forEach(ym => {
        ...
        html += `<tr>
          <td><b>${E.monthLabelIT(ym)}</b></td>
          <td>${fmt(d.deposits)}</td>
          <td>${d.withdrawals ? fmt(d.withdrawals) : '—'}</td>
  ...
  function applyAbnImport(overwrite) {
    ...
    Object.entries(importState.edits).forEach(([ym, edits]) => {
      if (digest[ym]) Object.assign(digest[ym], edits);
    });
    ...
    const msg = overwrite
      ...
      : `Importati ${log.length} valori (dati esistenti preservati).`;
    alert(msg + '\n\n' + log.join('\n'));
    render();
  }
  ...
  function renderInsights(main) {
    ...
    // Collect expense + income data
    const expenseData = months.map(ym => {
      const exp = E.estimatedExpenses(data, ym);
      const inc = E.totalIncome(data, ym);
      const entry = E.getEntry(data, ym);
      return { ym, expenses: exp.value, actual: exp.actual, flagged: exp.flags && exp.flags.length > 0, income: inc, entry };
    }).filter(d => d.expenses != null && !d.flagged);

    // ---- Expense summary card ----
    const last6 = expenseData.slice(-6);
    const avgExpenses = last6.length ? E.r2(last6.reduce((s, d) => s + d.expenses, 0) / last6.length) : null;
    const avgIncome = last6.length ? E.r2(last6.reduce((s, d) => s + d.income, 0) / last6.length) : null;
    const avgSaved = avgIncome && avgExpenses ? E.r2(avgIncome - avgExpenses) : null;

    const summBody = el('div');
    summBody.innerHTML = `
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:12px;text-align:center">
        <div class="metric-box"><div class="metric-label">Spesa media (6m)</div><div class="metric-value">${avgExpenses != null ? fmt(avgExpenses) : '—'}</div></div>
        <div class="metric-box"><div class="metric-label">Reddito medio (6m)</div><div class="metric-value">${avgIncome != null ? fmt(avgIncome) : '—'}</div></div>
        <div class="metric-box"><div class="metric-label">Risparmio medio (6m)</div><div class="metric-value" style="color:${avgSaved > 0 ? 'var(--pos)' : 'var(--neg)'}">${avgSaved != null ? fmt(avgSaved) : '—'}</div></div>
        <div class="metric-box"><div class="metric-label">Savings rate (6m)</div><div class="metric-value">${avgIncome && avgSaved ? ((avgSaved / avgIncome) * 100).toFixed(1) + '%' : '—'}</div></div>
      </div>`;
    main.appendChild(card('Riepilogo finanziario', summBody, 'Medie calcolate sugli ultimi 6 mesi di dati validi (mesi con flag esclusi). La savings rate è la quota di reddito che non viene spesa.'));

    // ---- Expense trend chart ----
    const trendBody = el('div');
    const cvExpTrend = document.createElement('canvas'); cvExpTrend.id = 'c-imp-expenses';
    trendBody.appendChild(cvExpTrend);
    main.appendChild(card('Spese vs Reddito nel tempo', trendBody, 'Confronta le tue spese mensili (barre rosse) con il reddito (linea blu). La differenza è quanto risparmi/investi ogni mese.'));

    const chartMonths = expenseData.slice(-12);
    const trendCfg = {
      type: 'bar',
      data: {
        labels: chartMonths.map(d => E.monthLabelIT(d.ym)),
        datasets: [
          { label: 'Spese', data: chartMonths.map(d => d.expenses), backgroundColor: 'rgba(231,76,60,0.6)', order: 2 },
          { label: 'Reddito', data: chartMonths.map(d => d.income), type: 'line', borderColor: '#3498db', backgroundColor: 'transparent', pointRadius: 3, order: 1 },
        ],
      },
      options: Object.assign(baseLineOpts(), { plugins: { legend: { display: true }, tooltip: { callbacks: { label: c => c.dataset.label + ': ' + fmt(c.parsed.y) } } } }),
    };
    makeChart('c-imp-expenses', trendCfg);

    // ---- Spending categories (from ABN transactions if available) ----
    renderCategoryInsights(main);

    // ---- Investment flow insights ----
    const invBody = el('div');
    const invMonths = months.slice(-12);
    const contribData = invMonths.map(ym => {
      const pc = E.portfolioContributions(data, ym);
      const mg = E.portfolioMarketGrowth(data, ym);
      return { ym, contrib: pc, market: mg };
    });

    const cvInv = document.createElement('canvas'); cvInv.id = 'c-imp-invest';
    invBody.appendChild(cvInv);
    main.appendChild(card('Flussi di investimento', invBody, 'Mostra quanto hai versato (contributi) vs quanto hanno reso i mercati ogni mese. Verde = i mercati hanno lavorato per te.'));

    makeChart('c-imp-invest', {
      type: 'bar',
      data: {
        labels: contribData.map(d => E.monthLabelIT(d.ym)),
        datasets: [
          { label: 'Contributi', data: contribData.map(d => d.contrib), backgroundColor: 'rgba(52,152,219,0.6)' },
          { label: 'Mercato', data: contribData.map(d => d.market), backgroundColor: contribData.map(d => d.market >= 0 ? 'rgba(46,204,113,0.6)' : 'rgba(231,76,60,0.6)') },
        ],
      },
    ...
    // ---- Salary trend ----
    const salaryData = months.map(ym => {
      const e = E.getEntry(data, ym);
      return { ym, salary: e ? e.salaryNet : 0, extra: e ? (e.extraSalary || 0) : 0 };
    }).filter(d => d.salary > 0);

    if (salaryData.length > 1) {
      const salBody = el('div');
      const cvSal = document.createElement('canvas'); cvSal.id = 'c-imp-salary';
      salBody.appendChild(cvSal);
      main.appendChild(card('Andamento stipendio', salBody, 'Mostra il tuo stipendio netto mensile nel tempo. Le barre arancioni sono le extra (13ª, 14ª, bonus).'));

      makeChart('c-imp-salary', {
        type: 'bar',
        data: {
          labels: salaryData.map(d => E.monthLabelIT(d.ym)),
          datasets: [
            { label: 'Stipendio netto', data: salaryData.map(d => d.salary), backgroundColor: 'rgba(52,152,219,0.7)' },
            { label: 'Extra (bonus)', data: salaryData.map(d => d.extra || null), backgroundColor: 'rgba(230,126,34,0.7)' },
          ],
        },
        options: Object.assign(baseLineOpts(), {
          scales: { x: { stacked: true }, y: { stacked: true, ticks: { callback: v => fmt(v) } } },
  ...
  function renderCategoryInsights(main) {
    // Try to categorize from stored entries with expensesActual or from importState
    const catTotals = {};
    let hasCategoryData = false;

    // Source 1: live import data with categories
    if (importState && importState.abnDigest) {
      Object.values(importState.abnDigest).forEach(mo => {
        if (mo.categories && !mo.partial) {
          hasCategoryData = true;
          Object.entries(mo.categories).forEach(([cat, val]) => {
    ...
    if (!hasCategoryData || Object.keys(catTotals).length === 0) return;

    const sorted = Object.entries(catTotals).sort((a, b) => b[1] - a[1]);
    const total = sorted.reduce((s, [, v]) => s + v, 0);
    const colors = ['#e74c3c', '#e67e22', '#f1c40f', '#2ecc71', '#1abc9c', '#3498db', '#9b59b6', '#34495e', '#95a5a6', '#d35400', '#c0392b', '#16a085', '#27ae60', '#2980b9'];

    const catBody = el('div');
    catBody.style.cssText = 'display:flex;gap:20px;flex-wrap:wrap;align-items:flex-start';

    // Donut chart
    const chartWrap = el('div');
    chartWrap.style.cssText = 'flex:1;min-width:250px;max-width:350px';
    const cvCat = document.createElement('canvas'); cvCat.id = 'c-imp-categories';
    chartWrap.appendChild(cvCat);
    catBody.appendChild(chartWrap);

    // Table
    const tableWrap = el('div');
    tableWrap.style.cssText = 'flex:1;min-width:250px';
    let tbl = '<table class="data-table" style="width:100%"><tr><th>Categoria</th><th>Totale</th><th>%</th></tr>';
    sorted.forEach(([cat, val]) => {
      const pct = ((val / total) * 100).toFixed(1);
      tbl += `<tr><td>${escapeHtml(cat)}</td><td>${fmt(E.r2(val))}</td><td>${pct}%</td></tr>`;
    });
    tbl += `<tr style="font-weight:bold"><td>Totale</td><td>${fmt(E.r2(total))}</td><td>100%</td></tr></table>`;
    tableWrap.innerHTML = tbl;
    catBody.appendChild(tableWrap);

    main.appendChild(card('Categorie di spesa', catBody, 'Classificazione automatica delle uscite dal conto ABN AMRO per categoria (supermercati, utenze, trasporti, ecc.). Basata sulla descrizione della transazione — non perfetta ma indicativa.'));

    makeChart('c-imp-categories', {
      type: 'doughnut',
      data: {
        labels: sorted.map(([cat]) => cat),
        datasets: [{ data: sorted.map(([, v]) => E.r2(v)), backgroundColor: sorted.map((_, i) => colors[i % colors.length]) }],
      },
      options: {
        responsive: true,
        plugins: {
  ...
  function ensureFireSim() {
    ...
    data.fireSim = {
      ...
      profile: {
        ...
        statePensionAge: f.pensionStartAge || 67, // one source of truth with the FIRE tab (editable)
        endAge: 90,
  ...
  /* --------------------------- chart options -------------------------- */
  // Chart.js plugin: dashed vertical marker + label for each milestone whose
  // month appears on the x-axis (spec: milestones on all time charts). Charts
  // opt in via options.milestoneMarkers = [{index,label}] — plain data, so it
  // survives the JSON clone used by the enlarge modal.
  const milestonePlugin = {
    id: 'fdMilestones',
    afterDatasetsDraw(chart) {
      const ms = chart.options && chart.options.milestoneMarkers;
      if (!ms || !ms.length || !chart.chartArea) return;
      const { ctx, chartArea, scales } = chart;
      const x = scales && scales.x; if (!x || !x.getPixelForValue) return;
      ms.forEach(m => {
        const px = x.getPixelForValue(m.index);
        if (!isFinite(px) || px < chartArea.left || px > chartArea.right) return;
        ctx.save();
        ctx.strokeStyle = 'rgba(155,89,182,.55)'; ctx.setLineDash([4, 4]); ctx.lineWidth = 1;
        ctx.beginPath(); ctx.moveTo(px, chartArea.top); ctx.lineTo(px, chartArea.bottom); ctx.stroke();
        ctx.setLineDash([]);
        ctx.fillStyle = 'rgba(155,89,182,.9)'; ctx.font = '10px sans-serif'; ctx.textAlign = 'left';
        ctx.fillText(m.label, Math.min(px + 4, chartArea.right - 70), chartArea.top + 10);
        ctx.restore();
      });
    },
  };
  if (typeof Chart !== 'undefined' && Chart.register) Chart.register(milestonePlugin);

  function withMilestones(options, months) {
    const marks = (data.settings.milestones || [])
      .map(m => ({ index: months.indexOf(m.date), label: m.label }))
      .filter(m => m.index >= 0);
    if (marks.length) options.milestoneMarkers = marks;
    return options;
  }
  ...
  window.FD = { get data() { return data; }, load, save, render, seedDemo, go, openEntry: openEntryForm, callModel, aiCfg, exportPayload };
})();
